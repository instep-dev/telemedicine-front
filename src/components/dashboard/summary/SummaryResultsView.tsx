...
export type SummaryRole = "doctor" | "nurse" | "patient";
type EditableField = "subjective" | "objective" | "assessment" | "plan";

const SOAP_LABELS: Record<SummaryRole, Record<EditableField, string>> = {
  doctor: {
  ...
  patient: {
    ...
    objective: "Objective (Physical Findings)",
...
function AiStatusBadge({ status }: { status: string | null }) {
  if (status === "SUCCESS")
    return (
      <Badge className="gap-1 bg-green-500/10 text-green-400 border-green-500/20 pointer-events-none">
  ...
  if (status === "FAILED")
    return (
      <Badge className="gap-1 bg-red-500/10 text-red-400 border-red-500/20 pointer-events-none">
        <XCircleIcon size={12} />
        AI Failed
      </Badge>
    );
  return <Badge variant="outline" className="pointer-events-none">-</Badge>;
}
      ...
      <div className="rounded-lg border border-cultured bg-card">
        <div className="px-6 pt-6 pb-2">
          ...
          <p className="text-sm text-accent mt-1">
            The SOAP note for this session has not been created or cannot be accessed.
          </p>
        </div>
...
export type SummaryResultsViewProps = {
  note: SoapNoteDto;
  role: SummaryRole;
  backHref: string;
  backLabel: string;
...
export function SummaryResultsView({
  note,
  role,
  backHref,
  backLabel,
  ...
  onFinalize,
  isFinalizing = false,
}: SummaryResultsViewProps) {
  const isDoctor = role === "doctor";
  const isPatient = role === "patient";
  const soapLabels = SOAP_LABELS[role];
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-accent">
            {isDoctor ? (
              <>AI details for session <span className="text-white">{note.consultationSessionId}</span></>
            ) : isPatient ? (
              <>All Details for your consultations <span className="text-white">{note.patientName ?? "-"}</span></>
            ) : (
              <>All details for consultation <span className="text-white">{note.patientName ?? "-"}</span> and dr. <span className="text-white">{note.doctorName ?? "-"}</span></>
            )}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {isDoctor && (
            note.isFinalized ? (
              <Badge className="gap-1 px-3 py-1.5 bg-green-500/10 text-green-400 border-green-500/20 text-sm pointer-events-none">
          ...
          {!isDoctor && (
            note.isFinalized ? (
              <Badge className="gap-1 px-3 py-1.5 bg-green-500/10 text-green-400 border-green-500/20 text-sm pointer-events-none">
                <CheckCircleIcon size={14} />
                {isPatient ? "Verified" : "Published to Patient"}
              </Badge>
            ) : (
              <Badge variant="secondary" className="px-3 py-1.5 text-sm pointer-events-none">
      ...
      {/* Status banners */}
      {isDoctor && note.aiStatus !== "SUCCESS" && (
        ...
        <div className="rounded-lg border border-cultured bg-card">
          ...
          <div className="px-6 pb-6 space-y-4">
            <div className="flex items-start gap-3">
              <div className="rounded-lg bg-blue-500/10 border border-blue-950 p-2 text-primary">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between rounded-lg border border-cultured bg-card/50 p-3">
                ...
                <span className="font-medium text-white">{note.aiModel ?? "Gemini Flash 3"}</span>
              </div>
              <div className="flex justify-between rounded-lg border border-cultured bg-card/50 p-3">
            ...
            {isDoctor && (
              <div className="flex items-center gap-2 text-xs border rounded-lg p-2 border-yellow-950 bg-yellow-500/10 text-yellow-600">
                <WarningCircleIcon size={14} />
                Manual review required before finalization.
              </div>
            )}
            {isPatient && (
              <div className="flex items-center gap-2 text-xs text-accent border rounded-lg p-2 border-cultured">
                <WarningCircleIcon size={14} />
        ...
        <div className="mb-3 flex items-center justify-between gap-2">
          ...
          {isDoctor && (
            <Badge variant="destructive" className="text-xs">
              <WarningCircleIcon /> Only the doctor can edit
            </Badge>
          )}
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="px-6 pt-6 pb-4">
            ...
            <p className="text-sm text-accent mt-1">
              {isPatient
                ...
                : "Checklist for the medical team."}
            </p>
          </div>
          <div className="px-6 pb-6 grid gap-3 text-sm sm:grid-cols-3">
            <div className="flex items-start gap-3 rounded-lg border border-cultured bg-card/50 p-3">
              ...
              <span className="text-accent">
                {isPatient
                  ? "Follow the treatment instructions as per the plan above."
                  : "Re-verify the patient's allergy history."}
              </span>
            </div>
            <div className="flex items-start gap-3 rounded-lg border border-cultured bg-card/50 p-3">
              ...
              <span className="text-accent">
                {isPatient
                  ...
                  : "Schedule a follow-up as recommended."}
              </span>
            </div>
            <div className="flex items-start gap-3 rounded-lg border border-cultured bg-card/50 p-3">
              ...
              <span className="text-accent">
                {isPatient
                  ? "Save these results as a reference for your health records."
                  : note.isFinalized
